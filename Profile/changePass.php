<?php
    ...
    if($category == 1)
    {
        ...
        else
        {
            ...
            if(password_verify($_POST['currPass'], $User['Password']))
            {
                if($newPass == $conNewPass)
                {
                    ...
                    $sql = "UPDATE farmer SET Password='$conNewPass', Hash='$newHash' WHERE Hash='$currHash';";
        ...
        $sql = "SELECT * FROM buyer WHERE Username='$user'";
        $result = mysqli_query($conn, $sql);
        $num_rows = mysqli_num_rows($result);
        ...
        else
        {
            ...
            if(password_verify($_POST['currPass'], $User['Password']))
            {
                if($newPass == $conNewPass)
                {
                    $conNewPass = dataFilter(password_hash($_POST['conNewPass'], PASSWORD_BCRYPT));
                    $currHash = $_SESSION['Hash'];
                    $sql = "UPDATE buyer SET Password='$conNewPass', Hash='$newHash' WHERE Hash='$currHash';";
